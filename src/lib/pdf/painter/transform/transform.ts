...
export class Transform {
    ...
    private async getAnnotations(): Promise<Annotation[]> {
        ...
        // Import PDFPageProxy type from pdfjs-dist
        // @ts-ignore: Import type only
        import type { PDFPageProxy } from 'pdfjs-dist/types/src/display/api';

        const annotationsPromises = Array.from({ length: numPages }, (_, i) =>
            pdfDocument.getPage(i + 1).then((page: PDFPageProxy) => {
                const _pageViewer = pdfViewer.getPageView(i)
                return page.getAnnotations().then((annotations: any[]) =>
