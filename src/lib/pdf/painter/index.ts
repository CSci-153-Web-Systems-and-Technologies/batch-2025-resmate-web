...
import { EditorFreeHand } from './editor/editor-free-hand'
import { EditorFreeHighlight } from './editor/editor-free-highlight'
import { EditorFreeText } from './editor/editor-free-text'
...
export class Painter {
    ...
    constructor({
        ...
        this.selector = new Selector({
            ...
            onChanged: async (id, groupString, _rawAnnotationStore, konvaClientRect, transformerRect) => {
                ...
                const annotationStore = this.store.annotation(id)
                if (annotationStore) {
                    this.onAnnotationChanged(annotationStore, transformerRect)
                }
            },
            onCancel: () => {
        ...
        this.webSelection = new WebSelection({
            ...
            onHighlight: (selection: { [key: string]: any }) => {
    ...
    private saveToStore(annotationStore: IAnnotationStore, isOriginal: boolean = false) {
        ...
        if (currentAnnotation) {
            ...
            console.warn('No matching annotation definition found for pdfjsType:', annotationStore.pdfjsType)
        }
    }
    ...
    private updateStore(id: string, updates: Partial<IAnnotationStore>) {
        const updatedAnnotation = this.store.update(id, updates)
        if (updatedAnnotation) {
            this.onAnnotationChange(updatedAnnotation)
        }
    }
     ...
     * @returns 编辑器实例
     */
    private findEditor(pageNumber: number, editorType: AnnotationType): Editor | null {
        return this.editorStore.get(`${pageNumber}_${editorType}`) ?? null
    }
    ...
    private reDrawAnnotation(pageNumber: number): void {
        ...
        annotationStores.forEach(annotationStore => {
            ...
            if (!storeEditor) {
                ...
                if (konvaCanvasStore && annotationDefinition) {
