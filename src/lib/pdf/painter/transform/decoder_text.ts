...
import { createDocumentIcon } from '../../utils/document-icon'
...
export class TextDecoder extends Decoder {
    ...
    public decodePdfAnnotation(annotation: TextAnnotation, allAnnotations: Annotation[]) {
        if (annotation.inReplyTo) {
            throw new Error('TextAnnotation is a reply annotation');
        }
        const color = convertToRGB(annotation.color?.components!)
